...
/**
 * iMessage channel for GAKRCLI — direct chat.db + AppleScript.
 *
 ...
 * support. State in ~/.gakrcli/channels/imessage/access.json, managed by the
...
const STATE_DIR = process.env.IMESSAGE_STATE_DIR ?? join(homedir(), '.gakrcli', 'channels', 'imessage')
...
// GAKRCLI can already Read+paste file contents, so this isn't a new exfil
// channel for arbitrary paths — but the server's own state is the one thing
// GAKRCLI has no reason to ever send. No inbox carve-out: iMessage attachments
...
function checkApprovals(): void {
  ...
  for (const senderId of files) {
    ...
    const err = sendText(chatGuid, "Paired! Say hi to GAKRCLI.")
