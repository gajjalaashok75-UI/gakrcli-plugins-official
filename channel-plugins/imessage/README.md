...
Connect iMessage to your GAKRCLI assistant. Reads `~/Library/Messages/chat.db` directly for history, search, and new-message detection; sends via AppleScript to Messages.app. No external server, no background process to keep alive.
...
```
/plugin install imessage@gakrcli-plugins-official
```
...
```sh
gakrcli --channels plugin:imessage@gakrcli-plugins-official
```
...
Handles are phone numbers (`+15551234567`) or Apple ID emails (`them@icloud.com`). If you're not sure what you want, ask GAKRCLI to review your setup.
...
| --- | --- | --- |
| `IMESSAGE_APPEND_SIGNATURE` | `true` | Appends `\nSent by GAKRCLI` to outbound messages. Set to `false` to disable. |
...
| `IMESSAGE_STATE_DIR` | `~/.gakrcli/channels/imessage` | Override where `access.json` and pairing state live. |
