...
Connect a Telegram bot to your GAKRCLI with an MCP server.

The MCP server logs into Telegram as a bot and provides tools to GAKRCLI to reply, react, or edit messages. When you message the bot, the server forwards the message to your GAKRCLI session.
...
Writes `TELEGRAM_BOT_TOKEN=...` to `~/.gakrcli/channels/telegram/.env`. You can also write that file by hand, or set the variable in your shell environment — shell takes precedence.
...
Pairing is for capturing IDs. Once you're in, switch to `allowlist` so strangers don't get pairing-code replies. Ask GAKRCLI to do it, or `/telegram:access policy allowlist` directly.
...
Inbound photos are downloaded to `~/.gakrcli/channels/telegram/inbox/` and the
