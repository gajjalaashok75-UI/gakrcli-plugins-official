...
| Config file | `~/.gakrcli/channels/telegram/access.json` |
...
```
/telegram:access set mentionPatterns '["^hey gakrcli\\b", "\\bassistant\\b"]'
```
...
`~/.gakrcli/channels/telegram/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
