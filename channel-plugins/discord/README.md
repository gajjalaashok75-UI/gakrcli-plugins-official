...
Connect a Discord bot to your GAKRCLI with an MCP server.

When the bot receives a message, the MCP server forwards it to GAKRCLI and provides tools to reply, react, and edit messages.
...
Install the plugin:
```
/plugin install discord@gakrcli-plugins-official
/reload-plugins
```
...
Writes `DISCORD_BOT_TOKEN=...` to `~/.gakrcli/channels/discord/.env`. You can also write that file by hand, or set the variable in your shell environment — shell takes precedence.
...
```sh
gakrcli --channels plugin:discord@gakrcli-plugins-official
```

**7. Pair.**

With GAKRCLI running from the previous step, DM your bot on Discord — it replies with a pairing code. If the bot doesn't respond, make sure your session is running with `--channels`. In your GAKRCLI session:
...
Pairing is for capturing IDs. Once you're in, switch to `allowlist` so strangers don't get pairing-code replies. Ask GAKRCLI to do it, or `/discord:access policy allowlist` directly.
...
| `download_attachment` | Download all attachments from a specific message by ID to `~/.gakrcli/channels/discord/inbox/`. Returns file paths + metadata. Use when `fetch_messages` shows a message has attachments. |
...
Downloads land in `~/.gakrcli/channels/discord/inbox/`.
