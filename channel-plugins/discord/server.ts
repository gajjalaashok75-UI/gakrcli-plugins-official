#!/usr/bin/env bun
/**
 * Discord channel for GAKRCLI.
 *
 * Self-contained MCP server with full access control: pairing, allowlists,
 * guild-channel support with mention-triggering. State lives in
 * ~/.gakrcli/channels/discord/access.json — managed by the /discord:access skill.
 *
...
const STATE_DIR = process.env.DISCORD_STATE_DIR ?? join(homedir(), '.gakrcli', 'channels', 'discord')
const ACCESS_FILE = join(STATE_DIR, 'access.json')
const APPROVED_DIR = join(STATE_DIR, 'approved')
const ENV_FILE = join(STATE_DIR, '.env')

// Load ~/.gakrcli/channels/discord/.env into process.env. Real env wins.
...
// upload. GAKRCLI can already Read+paste file contents, so this isn't a new
  ...
  for (const senderId of files) {
    ...
    void (async () => {
      try {
        ...
        if ('send' in ch) {
          await ch.send("Paired! Say hi to GAKRCLI.")
        }
        rmSync(file, { force: true })
      } catch (err) {
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  mcp.notification({
    ...
    process.stderr.write(`discord channel: failed to deliver inbound to GAKRCLI: ${err}\n`)
  })
}
