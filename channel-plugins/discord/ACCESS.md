...
| Config file | `~/.gakrcli/channels/discord/access.json` |
...
```
/discord:access set mentionPatterns '["^hey gakrcli\\b", "\\bassistant\\b"]'
```
...
`~/.gakrcli/channels/discord/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
