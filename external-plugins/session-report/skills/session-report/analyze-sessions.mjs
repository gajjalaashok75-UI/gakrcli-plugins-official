 ...
 * analyze-sessions.js
 *
 * Scans ~/.gakrcli/projects/**.jsonl transcript files and reports token usage,
 ...
 *    to the filename label (`agent-a<label>-<hash>.jsonl` â†’ label) or "fork".
...
const ROOT = flag('--dir', path.join(os.homedir(), '.gakrcli', 'projects'))
...
function promptPreview(text, slashCmd) {
  ...
  const t = text
    ...
    .replace(/\s+/g, ' ')
    .trim()
  return t.length > 240 ? t.slice(0, 237) + 'â€¦' : t
}

// Â±2 user messages around a given prompt, with the api-call count that
...
async function main() {
  ...
  for (const { p, info } of files) {
    ...
    n++
    if (!AS_JSON && n % 200 === 0) {
      process.stderr.write(`\r  scanned ${n}/${files.length} filesâ€¦`)
    }
  }
  if (!AS_JSON)
...
function printText({ overall, perProject, perSubagent, perSkill }) {
  ...
  const hr = () => line('â”€'.repeat(78))

  line()
  line(`GAKRCLI session analysis - ${ROOT}`)
  ...
  if (overall.cacheBreaks.length > TOP_N)
    line(`  â€¦ ${overall.cacheBreaks.length - TOP_N} more`)
  ...
  if (projects.length > TOP_N)
    line(`  â€¦ ${projects.length - TOP_N} more projects`)
  ...
  if (skills.length > TOP_N) line(`  â€¦ ${skills.length - TOP_N} more`)
  line()
}
...
function printBlock(title, s, indent = '') {
  ...
  if (topSkills.length)
    console.log(
      `${indent}  skills: ${topSkills.map(([k, v]) => `${k}x${v}`).join(', ')}`,
    )
}
