...
This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`. Add to gakr Desktop or any MCP-compatible agent orchestrator so other agents can query the graph live.

To configure in gakr Desktop, add to `gakr_desktop_config.json`:
```json
{
  "mcpServers": {
    "graphify": {
...
## For native gakrcli.md integration

Run once per project to make graphify always-on in gakrcli Code sessions:

```bash
graphify gakrcli install
```

This writes a `## graphify` section to the local `gakrcli.md` that instructs gakrcli to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.

```bash
graphify gakrcli uninstall  # remove the section
```
