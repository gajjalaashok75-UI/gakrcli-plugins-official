...
Use **data-extraction** skill for ingesting external platform data into pipelines:

| Data Source Category | Platforms | Integration Pattern | Use Cases |
|---------------------|-----------|---------------------|-----------|
| **E-commerce Data** | Amazon, eBay, Etsy (~1.2s) | Scheduled batch extraction → S3/Data Lake | Product catalogs, pricing trends, reviews |
| **Social Media** | Reddit, Twitter/X (~600ms) | Streaming API → Kafka → Data Warehouse | Sentiment analysis, trend detection |
| **Developer Platforms** | GitHub, Stack Overflow (~250ms) | API polling → ETL pipeline | Code metrics, community insights |
| **Financial Markets** | CoinGecko, FRED (~400ms) | Real-time API → streaming pipeline | Market data, economic indicators |
| **Academic Research** | arXiv, PubMed (~500ms) | Bulk download → data lake | Research datasets, citation networks |
| **Business Intelligence** | G2, Capterra, ProductHunt (~1s) | Daily batch → data warehouse | Competitive intelligence, market trends |

**Example data pipeline workflows:**
- E-commerce analytics: `data-extraction` (Amazon, eBay) → `data-engineer` (ETL to Snowflake) → `data-scientist` (analysis)
- Social sentiment pipeline: `data-extraction` (Reddit, Twitter) → Kafka → Spark Streaming → BigQuery
- Market intelligence: `data-extraction` (financial APIs) → Airflow orchestration → data warehouse → BI dashboards
