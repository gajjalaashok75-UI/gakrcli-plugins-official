...
### Data Collection for Startup Analysis

Use **data-extraction** skill to gather competitive intelligence and market data:

| Analysis Type | Data Sources | Extraction Methods | Startup Applications |
|---------------|--------------|-------------------|---------------------|
| **Competitive Pricing** | ProductHunt, G2, Capterra (~1s) | HTML parsing | Competitor pricing models, feature comparison |
| **Market Trends** | GitHub, Stack Overflow (~250ms) | REST API | Technology adoption, developer sentiment |
| **Product Reviews** | G2, Capterra, Amazon (~1s) | Browser automation | Customer pain points, feature requests |
| **Industry News** | News aggregators, Medium (~500ms) | RSS + API | Market movements, funding announcements |
| **Financial Data** | CoinGecko, FRED (~400ms) | REST API | Market indicators, economic trends |
| **Social Sentiment** | Reddit, Twitter/X (~600ms) | API + scraping | Brand perception, community feedback |

**Example startup workflows:**
- Market sizing: `data-extraction` (industry APIs, reports) → `startup-analyst` (TAM/SAM/SOM calculation)
- Competitive analysis: `data-extraction` (G2, Capterra, ProductHunt) → `startup-analyst` (positioning strategy)
- Technology trends: `data-extraction` (GitHub, Stack Overflow) → `startup-analyst` (tech stack decisions)
