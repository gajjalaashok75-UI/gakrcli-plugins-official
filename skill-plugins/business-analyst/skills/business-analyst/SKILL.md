...
Use **data-extraction** skill to gather competitive and market intelligence:

| Analysis Type | Data Sources | Extraction Methods | Business Applications |
|---------------|--------------|-------------------|----------------------|
| **Competitive Pricing** | Amazon, eBay, e-commerce (~1.2s) | Browser automation | Price monitoring, competitive positioning |
| **Product Reviews** | G2, Capterra, Amazon (~1s) | HTML parsing | Feature gap analysis, customer pain points |
| **Market Sentiment** | Reddit, Twitter/X, Quora (~600ms) | API + scraping | Brand perception, trend identification |
| **Technology Adoption** | GitHub, Stack Overflow (~250ms) | REST API | Technology trends, developer preferences |
| **Financial Metrics** | TradingView, MacroTrends (~800ms) | Browser automation | Competitor financial analysis, market sizing |
| **Industry News** | News aggregators, Medium (~500ms) | RSS + API | Market intelligence, regulatory changes |

**Example BI workflows:**
- Competitive analysis: `data-extraction` (G2, Capterra) → `business-analyst` (analysis) → `chart-visualization` (dashboards)
- Market sizing: `data-extraction` (industry data APIs) → `data-analysis` (modeling) → `business-analyst` (strategic recommendations)
- Sentiment tracking: `data-extraction` (social media) → `data-scientist` (sentiment analysis) → `business-analyst` (insights)
